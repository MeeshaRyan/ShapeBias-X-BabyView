...
combined_adult <- filter(combined_diarised_select2,
...
write.csv(bv_adult_nouns_child_age, "bv_adult_nouns_child_age.csv", row.names = FALSE)
...
what if we just read in the csv file?
```{r}
library(tidyverse)

all_bv_cdis <- read_csv("/Users/meesha/Desktop/ShapeBias-X-BabyView/BV-Eng-CDIs/bv_cdis_combined.csv")

all_produced_bv_cdis <- all_bv_cdis %>% 
  filter(all_bv_cdis$value == "produces")

# #Split the string by underscore
# parts <- strsplit(all_produced_bv_cdis$variable, ".")[[1]]
# 
# # Assign parts to variables
# all_produced_bv_cdis$lemma <- parts[1]
# all_produced_bv_cdis$other <- parts[2]
# 
...
# Build the label (tag) from each filename
tags1 <- str_match(basename(all_produced_bv_cdis$variable), extract_pattern)[, 2]

# (optional) sanity check
if (any(is.na(tags1)))

# Attach names so map_dfr can use them for the .id column
names(all_produced_bv_cdis$variable) <- tags1
